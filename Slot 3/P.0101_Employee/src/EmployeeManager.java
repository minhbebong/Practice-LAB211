...
public class EmployeeManager {
    ...
    public void addEmployee(ArrayList<Employee> empList) throws ParseException {
        String id = "";
        while (true) {
            // ID must have 3 digits
            id = checkInput.getString("Enter ID: ", "Invalid ID", "^(\\d{1,3})$", null);
        ...
        String firstName = checkInput.getString("Enter first name: ", "Invalid first name",
                "^[A-Z][a-zA-Z ]*$", null);
        String lastName = checkInput.getString("Enter last name: ", "Invalid last name",
                "^[A-Z][a-zA-Z ]*$", null);
        // phone number must start with 0 and have 10 digits
        String phone = checkInput.getString("Enter phone num: ", "Invalid phone num",
                "^0\\d{9}$", null);
        // email must have form abc@abc.com
        String email = checkInput.getString("Enter email: ", "Invalid email",
                "^\\w*@(\\w+[.])*\\w+$", null);
        // address: only accept letters
        String add = checkInput.getString("Enter address: ", "Invalid address",
                "^[a-zA-Z ]*$", null);
        // dob must match dd/mm/yyyy
        Date dob = checkInput.getDate("Enter date of birth: ", "Invalid dob",
                "dd/MM/yyyy", null);
        // gender choice is M or F only
        String sex = checkInput.getString("Enter sex (M/F): ", "Choose male (M) or female (F) only",
                "^(M|F)$", null);
        double salary = checkInput.getDouble("Enter salary: ", "Salary must be > 0",
                null);
        // agency name: only accept letters
        String agency = checkInput.getString("Enter agency: ", "Invalid agency",
                "^[a-zA-Z ]*$", null);
    ...
    public void updateEmployeeById(ArrayList<Employee> empList) throws ParseException {
        if (empList.isEmpty()) {
            System.out.println("List empty");
            return;
        }
        String id = checkInput.getString("Enter ID: ", "Invalid ID", "^(\\d{3})$", null);
        Employee employee = getEmployeeById(empList, id);

        // if employee object is found, require user to fill in each field
        if (employee != null) {
            while (true) {
                id = checkInput.getString("Enter new ID: ", "Invalid ID", "^(\\d{3})$", null);
            ...
            String firstName = checkInput.getString("Enter new first name: ", "Invalid first name",
                    "^[A-Z][a-zA-Z ]*$", "");
            String lastName = checkInput.getString("Enter new last name: ", "Invalid last name",
                    "^[A-Z][a-zA-Z ]*$", "");
            String phone = checkInput.getString("Enter new phone num: ", "Invalid phone num",
                    "^0\\d{9}$", "");
            String email = checkInput.getString("Enter new email: ", "Invalid email",
                    "^\\w*@(\\w+[.])*\\w+$", "");
            String add = checkInput.getString("Enter new address: ", "Invalid address",
                    "^[a-zA-Z ]*$", "");
            Date dob = checkInput.getDate("Enter new date of birth: ", "Invalid dob",
                    "dd/MM/yyyy", "");
            String sex = checkInput.getString("Enter new sex: ", "Choose male or female only",
                    "^(M|F|m|f)$", "");
            Double salary = checkInput.getDouble("Enter new salary: ", "Salary must be > 0",
                    "");
            String agency = checkInput.getString("Enter new agency: ", "Invalid agency",
                    "^[a-zA-Z ]*$", "");
    ...
    public void findEmployeeByName(ArrayList<Employee> empList) {
        ...
        String name = checkInput.getString("Enter name: ", "Invalid name", "^[a-zA-Z ]+$", null);
